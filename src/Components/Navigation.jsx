...
function Navbar() {
  ...
  return (
    <>
      <header className='w-full py-8 px-20 flex items-center justify-between'>
        ...
        <button className='xl:hidden cursor-pointer absolute top-10 right-20' onClick={() => {
          setIsMenuOpen(!isMenuOpen)
        }}>
          <FontAwesomeIcon icon={faBars} className='cursor-pointer' />
        </button>
        {/* Moble Menu */}
        <div className={`absolute xl:hidden left-0 top-24 py-7 w-full bg-white text-center flex-col items-center justify-center text-gray-800 ${isMenuOpen ? "opacity-100" : "opacity-0"} transition-all`}>
